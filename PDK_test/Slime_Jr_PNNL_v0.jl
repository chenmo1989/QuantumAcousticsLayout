...
function assemble_schematic_graph()
    ...
    attach!(g, p_feedline_node, rres_node1 => :feedline, 1mm, i=6, location=-1)
    attach!(g, p_feedline_node, rres_node2 => :feedline, 1mm, i=6, location=1)
    attach!(g, p_feedline_node, rres_node3 => :feedline, 2mm, i=6, location=1)
    attach!(g, p_feedline_node, rres_node4 => :feedline, 2mm, i=6, location=-1)

    return g
end

function TransmissionLine(g)
    p_feedline = Path(Point(-1250μm, 2100μm) - Point(650μm, 0μm); α0=270°, metadata=LayerVocabulary.METAL_NEGATIVE) # 650μm is the length of the launcher
    sty = launch!(p_feedline)
    straight!(p_feedline, 950μm, sty)
    turn!(p_feedline, 90°, 500μm)
    straight!(p_feedline, 2800μm)
    turn!(p_feedline, -90°, 500μm)
    straight!(p_feedline, 950μm, sty)
    launch!(p_feedline)
...
save(joinpath(@__DIR__, "Slime_Jr_PNNL_v0.gds"), artwork)